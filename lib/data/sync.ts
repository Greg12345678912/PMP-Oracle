 ...
 * Records a sync_jobs row for every run (success or failure).
 */
export async function syncPlayersToCache(scoring: 'ppr' | 'half_ppr' | 'standard' = 'ppr'): Promise<SyncResult> {
  ...
  const { data: jobRow } = await db
    .from('sync_jobs')
    .insert({ resource: 'players', provider: 'sleeper', status: 'running', started_at: startedAt })
    .select('id')
    .single()
  const jobId = (jobRow as { id: string } | null)?.id ?? null

  const finishJob = async (result: SyncResult) => {
    if (!jobId) return
  ...
  try {
    ...
    const result: SyncResult = { upserted, skipped, errors }
    await finishJob(result)
    return result
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err)
    const failed: SyncResult = { upserted: 0, skipped: 0, errors: [message] }
